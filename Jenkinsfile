    ...
    stages {
        ...
        stage('Deploy Integration Test') {
            steps {
                ...
                sh "./kubectl exec -n integration-test integration-${env.BUILD_ID} -c cypress -- \$(npm bin)/cypress run --spec cypress/integration/sample_spec.js --config-file cypress.integration.json"
            }
            post {
                always {
